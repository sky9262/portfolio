...
export const Projects = () => {
  ...
  const projects = ["ServerlessWeb", "ec2AutoScaling", "phishEye", "winLock", "gce-korea"];

  if (!expanded) {
    return (
      <div className="w-100 text-center mt-3">
        <Button onClick={() => setExpanded(true)}>{t(`Project.cta`)}</Button>
      </div>
    );
  }
  ...
  return (
    <div
      className={`${windowSize.containerClass} ${
        expanded
          ? styles["projects-container--show"]
          : styles["projects-container--hide"]
      } mt-3`}
    >
      ...
      <div className="row">
        {projects.map((project, index) => {
          ...
          const imgSrc2 = t(`Project.${project}.imgSrc2`);
          const url = t(`Project.${project}.url`);

          return (
            <div key={`project-${index}`} className="col-12 col-lg-6">
              <div
                className={`${styles["project__card"]} card-container mt-3 p-3`}
              >
                {!!imgSrc2 ? (
                  <div
                ...
                <div>
                  ...
                  {(!!primaryType || !!secondaryType) && (
                    <div>
                      <p className="badge bg-lilac rounded-pill m-0 me-2 text-black">
                        {primaryType}
                      </p>
                      <p className="badge bg-secondary rounded-pill m-0 w-auto">
                        {secondaryType}
                      </p>
                    </div>
                  )}
